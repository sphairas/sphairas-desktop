...
@MultiViewElement.Registration(
        displayName = "#DefaultPropertiesTopComponent.displayName",
        //        iconBase = "org/thespheres/betula/termreport/resources/betulatrep2_16.png",
        mimeType = ProviderReference.MIME,
        persistenceType = TopComponent.PERSISTENCE_ONLY_OPENED,
        preferredID = "DefaultPropertiesTopComponent",
        position = 1000)
@NbBundle.Messages({"DefaultPropertiesTopComponent.displayName=Basis-Eigenschaften",
    "DefaultPropertiesTopComponent.displayName.withProvider=Basis-Eigenschaften ({0})"})
public class DefaultPropertiesTopComponent extends AbstractTableElement implements MultiViewElement, Serializable {

    public static final String MIME = "application/app-resources-default-properties";
    public static final String RESOURCE = "default.properties";
    private ProviderReference env;
    private final Listener listener = new Listener();
    private final DefaultAppResourcesConfigTableModel model = DefaultAppResourcesConfigTableModel.create(MIME);
    private final javax.swing.JScrollPane scrollPane = new javax.swing.JScrollPane();

    @SuppressWarnings({"LeakingThisInConstructor",
        "OverridableMethodCallInConstructor"})
    public DefaultPropertiesTopComponent() {
        super();
    ...
    @SuppressWarnings({"LeakingThisInConstructor",
        "OverridableMethodCallInConstructor"})
    public DefaultPropertiesTopComponent(final Lookup lkp) {
        this();
...
//        });
    ...
    protected void updateName() {
        if (env != null) {
            Mutex.EVENT.writeAccess(() -> {
                final String name = NbBundle.getMessage(DefaultPropertiesTopComponent.class, "DefaultPropertiesTopComponent.displayName.withProvider", env.getProviderInfo().getDisplayName());
                setDisplayName(name);
                if (callback != null) {
    ...
    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        if (oi instanceof AppResourcesConfigOpenSupport.Env) {
            env = (AppResourcesConfigOpenSupport.Env) oi;
        } else {
