 ...
 */
//Portions Copyrighted 2015 boris.heithecker
...
final class TreeViewDropSupport implements DropTargetListener, Runnable {
    ...
    /**
     * Drop target asociated with the treeTable
     */
     ...
     * View manager.
     */
    protected NbSwingXTreeTableElement treeTableElement;
     ...
     */
    public TreeViewDropSupport(NbSwingXTreeTableElement view, JXTreeTable tree, boolean dropTargetPopupAllowed) {
        this.treeTableElement = view;
        this.treeTable = tree;
    ...
    private void checkStoredGlassPane() {
        ...
        if (!DropGlassPane.isOriginalPaneStored()) {
            Component comp = treeTable.getRootPane().getGlassPane();
            DropGlassPane.setOriginalPane(treeTable, comp, comp.isVisible());

            // set glass pane for paint selection line
            dropPane = DropGlassPane.getDefault(treeTable);
            treeTable.getRootPane().setGlassPane(dropPane);
            dropPane.revalidate();
    ...
    private void doDragOver(DropTargetDragEvent dtde) {
        ...
        int allowedDropActions = treeTableElement.getAllowedDropActions();
        ...
        if (tp == null) {
            ...
            dropNode = treeTableElement.model.getRootNode();
        ...
        Rectangle nodeArea = treeTableElement.getTree().getPathBounds(tp);
        int endPointX = nodeArea.x + nodeArea.width;
        int row = treeTable.getRowForPath(tp);
        ...
        if (nodeArea != null) {
            ...
            if (p.y <= (nodeArea.y + FUSSY_POINTING)) {
                ...
                if (row != 0 || !treeTable.isRootVisible()) {
                    // point above node
                    pointAt = DragDropUtilities.NODE_UP;

                    TreePath upPath = treeTable.getPathForRow(row - 1);

                    if ((upPath != null) && !upPath.equals(tp)) {
                        endPointX = Math.max(nodeArea.x + nodeArea.width,
                                treeTableElement.getTree().getPathBounds(upPath).x + treeTableElement.getTree().getPathBounds(upPath).width
                        );
                    }
                ...
                if (!treeTableElement.isExpanded(dropNode)) {
                    // point bellow node
                    pointAt = DragDropUtilities.NODE_DOWN;

                    TreePath downPath = treeTable.getPathForRow(row + 1);

                    if ((downPath != null) && !downPath.equals(tp)) {
                        endPointX = Math.max(nodeArea.x + nodeArea.width,
                                treeTableElement.getTree().getPathBounds(downPath).x + treeTableElement.getTree().getPathBounds(downPath).width
                        );
                    }
        ...
        if (((timer == null) || !timer.isRunning()) && (dropNode != null) && !dropNode.isLeaf()
                && !treeTableElement.isExpanded(dropNode)) {
            ...
            timer = new Timer(
                ...
                final public void actionPerformed(ActionEvent e) {
                    treeTableElement.expandNode(cn);
                }
            }
            );
        ...
        if (pointAt == DragDropUtilities.NODE_CENTRAL) {
            ...
            nodeArea = (Rectangle)nodeArea.createUnion (treeTable.getPathBounds (tp));*/
        }

        // back normal treeTableElement w/o any selecetion nor line
...
//                NodeRenderer.dragEnter(tp.getLastPathComponent());
    ...
    private void repaint(Rectangle r) {
        treeTable.repaint(r.x - 5, r.y - 5, r.width + 10, r.height + 10);
    }
    ...
    private void convertBoundsAndSetDropLine(final Line2D line) {
        ...
        Point p1 = SwingUtilities.convertPoint(treeTable, x1, y1, treeTable.getRootPane());
        Point p2 = SwingUtilities.convertPoint(treeTable, x2, y2, treeTable.getRootPane());
        line.setLine(p1, p2);
        if (null != dropPane) {
    ...
    public void dropActionChanged(DropTargetDragEvent dtde) {
        ...
        if (null != nodes) {
            int dropAction = ExplorerDnDManager.getDefault().getAdjustedDropAction(dtde.getDropAction(), treeTableElement.getAllowedDropActions()
            );

            for (int i = 0; i < nodes.length; i++) {
                if (((treeTableElement.getAllowedDropActions() & dropAction) == 0)
...
//            NodeRenderer.dragExit();
    ...
    private Node getNodeForDrop(Point p) {
        if (p != null) {
            TreePath tp = treeTable.getPathForLocation(p.x, p.y);
            if (null == tp) {
                //make the drop area a bit bigger at the end of the treeTable
                tp = treeTable.getPathForLocation(p.x, p.y - treeTable.getRowHeight() / 2);
            }
    ...
    public void drop(DropTargetDropEvent dtde) {
        ...
        try {
            ...
            if (DnDConstants.ACTION_LINK == dropAction && null != dragNodes) {
                ...
                DragDropUtilities.createDropFinishPopup(setPasteTypes).show(treeTable, Math.max(dtde.getLocation().x - 5, 0), Math.max(dtde.getLocation().y - 5, 0)
                );
...
//            TreeCellEditor tce = treeTable.getCellEditor();
//
...
//            }
    ...
    /**
     * @return The treeTable path to the node the cursor is above now or null if
     * no such node currently exists or if conditions were not satisfied to
     * continue with DnD operation.
     */
    TreePath getTreePath(DropTargetDragEvent dtde, int dropAction) {
        // check location
        Point location = dtde.getLocation();
        TreePath tp = treeTable.getPathForLocation(location.x, location.y);
        if (null == tp) {
            //make the drop area a bit bigger at the end of the treeTable
            tp = treeTable.getPathForLocation(location.x, location.y - treeTable.getRowHeight() / 2);
        }
    ...
    /**
     * Safe accessor to the drop target which is asociated with the treeTable
     */
    DropTarget getDropTarget() {
        if (dropTarget == null) {
            dropTarget = new DropTarget(treeTable, treeTableElement.getAllowedDropActions(), this, false);
        }
        return dropTarget;
    }
}
